...
class Transformer(nn.Module):
    ...
    def __init__(self, model, src_pad_idx, trg_pad_idx, trg_sos_idx, enc_voc_size, dec_voc_size, d_model, n_head, max_len,
    ...
    def forward(self, x, src, title):
        src_mask = self.make_src_mask(src)
        trg_mask = self.make_trg_mask(title)
        enc_src = self.encoder(x, src_mask)
        output = self.decoder(title, enc_src, trg_mask, src_mask)
        # output shape [batch_size, seq_len, dec_voc_size]
        return output
